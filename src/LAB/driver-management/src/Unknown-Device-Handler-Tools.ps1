...
function PNPInstallProcess {
    ...
    if ($ByID -eq "") {
        $ByID = "HardwareID"
        Write-Host "[i] PNPInstallProcess: Using [$byID] by default. < HardwareID | CompatibleID >" -ForegroundColor Cyan
    } else {
        Write-Host "[i] PNPInstallProcess: Using [$ByID]. < HardwareID | CompatibleID >" -ForegroundColor Cyan
    }
